...
void CheckboxWidget::draw(RuiMonitor &monitor, const Rect &showableArea) {
  ...
  Rect boxRect{positionPixel.x + 5,
               positionPixel.y + (positionPixel.h - checkboxLength) / 2,
  ...
  if (variable) {
    monitor.drawImage(boxRect, showableArea, boxTexture,
                      CheckboxWidget::ASSET_PATH);
  }
  monitor.drawText({boxRect.x + boxRect.w + 5, boxRect.y,
                    positionPixel.w - boxRect.w - 5, checkboxLength},
